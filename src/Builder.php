 ...
 * @LastEditTime: 2019-04-24 11:34:06
 */
...
{   
    ...
    // protected $selectSqlLimit = 'SELECT * FROM (SELECT tp_.*,ROWNUM AS NUMROW FROM (SELECT%FORCE%%DISTINCT% %FIELD% FROM %TABLE%%JOIN%%WHERE%%GROUP%%HAVING%%UNION%%ORDER%) tp_ ) %LIMIT%%COMMENT%'; 
    ...
    // SELECT * FROM 
    // (    
    //     SELECT ROWNUM RN, thinkphp.* FROM (  ) thinkphp WHERE ROWNUM <= 
    // ) WHERE RN > 
     ...
     * @return void
     */
    protected function parseLimitBegin(Query $query, $limit)
    {
        $limitStr = '';
     ...
     * @return void
     */
    protected function parseLimitEnd(Query $query, $limit)
    {
        $limitStr = '';
    ...
    protected function parseForce(Query $query, $index)
    {
        ...
        return sprintf(" /*+INDEX(%s %s)*/", $table, $index); // oracle 的
    }
