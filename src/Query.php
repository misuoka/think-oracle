...
class Query
{
    ...
    public function __call($method, $args)
    {
        if (isset(self::$extend[strtolower($method)])) {
            // 调用扩展查询方法
            array_unshift($args, $this); // 函数用于向数组插入新元素。新数组的值将被插入到数组的开头。
    ...
    public function fetchOci($oci = true)
    {
        $this->options['fetch_oci'] = $oci;
        return $this;
    }
     ...
     * @return $this|string
     */
    public function bind($value, $type = SQLT_CHR, $name = null)
    {
        if (is_array($value)) {
